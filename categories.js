      ...
      return `
        <article class="store-card ${state.focusedStoreId === store.id ? "is-active" : ""}">
          <div class="store-chip-row">
            <span class="store-type-pill">${capitalize(store.businessType)}</span>
            <span class="store-chip">${store.location}</span>
          </div>
          <div>
            <h3>${store.storeName}</h3>
            <p>${[...new Set(storeCategories)].join(" | ") || "No products added yet"}</p>
          </div>
          <div class="store-chip-row">
            <span class="store-chip">Prep ${store.prepTime}</span>
            <span class="store-chip">Min ${currency(store.minimumOrder || 0)}</span>
            <span class="store-chip">${distanceLabel}</span>
          </div>
          <div class="tag-row">
            ${normalizePaymentOptions(store.paymentOptions)
              .map((method) => `<span class="meta-pill">${method}</span>`)
              .join("")}
          </div>
          <button class="button button-outline button-small" data-focus-store="${store.id}" type="button">
            ${state.focusedStoreId === store.id ? "Showing products" : "View products"}
          </button>
        </article>
      `;
    })
...
function renderProducts() {
  ...
  container.innerHTML = list
    .map((product) => {
      ...
      const paymentLabel = store ? normalizePaymentOptions(store.paymentOptions).join(", ") : "Seller payment methods";
      ...
      return `
        <article class="product-card">
          ...
          <p class="tiny">Payments: ${paymentLabel}</p>
